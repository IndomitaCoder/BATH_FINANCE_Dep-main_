...
function useApproveTaxOffice(): [ApprovalState, () => Promise<void>] {
  ...
  let token: ERC20 = bathFinance.TOMB;
  // if (zappingToken === FTM_TICKER) token = bathFinance.FTM;
  // else if (zappingToken === TOMB_TICKER) token = bathFinance.TOMB;
  // else if (zappingToken === TSHARE_TICKER) token = bathFinance.TSHARE;
  ...
  const approvalState: ApprovalState = useMemo(() => {
    ...
    if (token === bathFinance.FTM) return ApprovalState.APPROVED;
  ...
  }, [currentAllowance, pendingApproval, token, bathFinance]);
