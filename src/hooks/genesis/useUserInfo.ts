...
const useUserInfoByPidAndAccount = (pId: Number, account: string) => {
  ...
  useEffect(() => {
    async function fetchUserInfo() {
      try {
        setStat(await bathFinance.getPoolUserInfoByPidAndAddress(pId, account));
      }
      catch (err) {
        console.error(err)
      }
...
export default useUserInfoByPidAndAccount;