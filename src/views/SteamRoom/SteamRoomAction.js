...
const SteamRoomAction = ({ poolInfo, pId }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      let res_p = await bathFinance.STEAMROOM.pendingTubs(pId, account)
  ...
  const onDepositClick = async () => {
    try {
      if (Number(inputValue) > 0) {
        if (poolInfo?.token) {
          let response = await lpContract.allowance(account, bathFinance.STEAMROOM.address)
          let allowedAmount = Number(getFullDisplayBalance(response, lpContract.decimal));
          if (allowedAmount < Number(inputValue)) {
            let response = await lpContract.approve(bathFinance.STEAMROOM.address, ethers.constants.MaxUint256);
          ...
          let steamRoomFarm = bathFinance.STEAMROOM.connect(signer);
          response = await steamRoomFarm.deposit(pId, amountBn)
          await addTransaction(response, {
  ...
  const onWithdrawClick = async () => {
    try {
      if (Number(inputValue) > 0) {
        const amountBn = parseUnits(Number(inputValue).toString(), poolInfo?.token.decimal)
        let steamRoomFarm = bathFinance.STEAMROOM.connect(signer);
        let response = await steamRoomFarm.withdraw(pId, amountBn);
        await addTransaction(response, {
          summary: `Withdraw ${lpContract.symbol} and TUB`,
          approval: {
            tokenAddress: lpContract.address,
            spender: account,
          },
        ...
        <Container maxWidth="lg">
          <Box mt={5} width={`100%`} style={{ display: "flex", flexDirection: 'column', backgroundColor: 'rgba(1,1,1,0.2)', padding: '50px 20px', borderRadius: '20px', backdropFilter: 'blur(15px)', margin: 0 }}>
            <div style={{ color: 'white', display: 'flex', flexFlow: 'wrap', flexDirection: 'column-reverse' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                <p style={{ margin: 0 }}>Pending TUB: &nbsp;</p>
                <p style={{ fontSize: 'larger', margin: 0 }}>{pendingBath + ' TUB'}</p>
              </div>
            </div>
...
export default SteamRoomAction;
