...
import usePoolInfoByPid from '../../hooks/farm3/usePoolInfo';
import { getFullDisplayBalance } from '../../utils/formatBalance';
import useWallet from 'use-wallet';
import GenesisAction from './GenesisAction';
import MiniLoader from '../../components/MiniLoader';
import useTotalAllocPoint from '../../hooks/farm3/useTotalAllocPoint';
...
const GenesisCard = ({ pId, PoolInfo }) => {
  ...
  const fetchData = async () => {
    try {
      const info = await tombFinance.SAUNA.poolInfo(pId)
      ...
      let res_t = await lpContract.balanceOf(tombFinance.SAUNA.address);
      setlpSymbol(res_s);
      settvl(res_t)
