...
import usePoolInfoByPid from '../../hooks/jacuzzi/usePoolInfo';
import { getFullDisplayBalance } from '../../utils/formatBalance';
import useWallet from 'use-wallet';
import JacuzziAction from './JacuzziAction';
import MiniLoader from '../../components/MiniLoader';
import useBATHFinance from '../../hooks/useBATHFinance';
import ERC20 from '../../tomb-finance/ERC20';
import { ethers } from 'ethers';

const JacuzziCard = ({ pId, PoolInfo }) => {
  const [isHovered, setIsHovered] = useState(false);
  const [isJacuzziActionOpen, setisJacuzziActionOpen] = useState(false)
  ...
  const fetchData = async () => {
    try {
      const info = await bathFinance.JACUZZI.poolInfo(pId)
      ...
      let res_t = await lpContract.balanceOf(bathFinance.JACUZZI.address);
      setlpSymbol(res_s);
      settvl(res_t)
      ...
      //APR & TVL
        ...
        let BathPerSecond = await bathFinance?.JACUZZI?.bathPerSecond();
      ...
      {poolInfo?.token ?
        <Box style={{ margin: 0 }}>
          <Card variant="outlined" style={styles}
            ...
            onClick={() => { setisJacuzziActionOpen(!isJacuzziActionOpen) }}
          >
            <CardContent>
          ...
          <div style={{ display: !isJacuzziActionOpen ? 'none' : 'block' }}>
            <JacuzziAction poolInfo={poolInfo} pId={pId} />
          </div>
        </Box>
