...
const JacuzziAction = ({ poolInfo, pId }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      let res_p = await bathFinance.JACUZZI.pendingBaths(pId, account)
      let res_d = await bathFinance.JACUZZI.userInfo(pId, account);
  ...
  const onDepositClick = async () => {
    try {
      if (Number(inputValue) > 0) {
        if (poolInfo?.token) {
          let response = await lpContract.allowance(account, bathFinance.JACUZZI.address)
          let allowedAmount = Number(getFullDisplayBalance(response, lpContract.decimal));
          if (allowedAmount < Number(inputValue)) {
            let response = await lpContract.approve(bathFinance.JACUZZI.address, ethers.constants.MaxUint256);
          ...
          let jacuzziFarm = bathFinance.JACUZZI.connect(signer);
          response = await jacuzziFarm.deposit(pId, amountBn)
          await addTransaction(response, {
  ...
  const onWithdrawClick = async () => {
    try {
      if (Number(inputValue) > 0) {
        const amountBn = parseUnits(Number(inputValue).toString(), poolInfo?.token.decimal)
        let jacuzziFarm = bathFinance.JACUZZI.connect(signer);
        let response = await jacuzziFarm.withdraw(pId, amountBn);
        await addTransaction(response, {
