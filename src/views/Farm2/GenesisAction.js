...
const GenesisAction = ({ poolInfo, pId }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      let res_p = await tombFinance.JACUZZI.pendingBaths(pId, account)
  ...
  const onDepositClick = async () => {
    try {
      if (Number(inputValue) > 0) {
        if (poolInfo?.token) {
          let response = await lpContract.allowance(account, tombFinance.JACUZZI.address)
          let allowedAmount = Number(getFullDisplayBalance(response, lpContract.decimal));
          if (allowedAmount < Number(inputValue)) {
            let response = await lpContract.approve(tombFinance.JACUZZI.address, ethers.constants.MaxUint256);
          ...
          let genesisFarm = tombFinance.JACUZZI.connect(signer);
