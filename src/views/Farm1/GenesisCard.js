...
import usePoolInfoByPid from '../../hooks/farm1/usePoolInfo';
import { getFullDisplayBalance } from '../../utils/formatBalance';
import useWallet from 'use-wallet';
import GenesisAction from './GenesisAction';
import MiniLoader from '../../components/MiniLoader';
import useTotalAllocPoint from '../../hooks/farm1/useTotalAllocPoint';
...
const GenesisCard = ({ pId, PoolInfo }) => {
  ...
  const fetchData = async () => {
    try {
      const info = await tombFinance.STEAMROOM.poolInfo(pId)
      ...
      let res_t = await lpContract.balanceOf(tombFinance.STEAMROOM.address);
      setlpSymbol(res_s);
      settvl(res_t)
