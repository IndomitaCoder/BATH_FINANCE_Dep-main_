...
const GenesisAction = ({ poolInfo, pId }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      let res_p = await tombFinance.STEAMROOM.pendingTubs(pId, account)
      console.log(res_p, "res_p");
      let res_d = await tombFinance.STEAMROOM.userInfo(pId, account);
  ...
  const onDepositClick = async () => {
    try {
      if (Number(inputValue) > 0) {
        if (poolInfo?.token) {
          let response = await lpContract.allowance(account, tombFinance.STEAMROOM.address)
          let allowedAmount = Number(getFullDisplayBalance(response, lpContract.decimal));
          if (allowedAmount < Number(inputValue)) {
            let response = await lpContract.approve(tombFinance.STEAMROOM.address, ethers.constants.MaxUint256);
          ...
          let genesisFarm = tombFinance.STEAMROOM.connect(signer);
