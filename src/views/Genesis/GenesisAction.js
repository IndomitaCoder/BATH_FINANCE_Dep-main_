...
const GenesisAction = ({ poolInfo, pId }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      let res_p = await bathFinance.GENESISFARM.pendingBaths(pId, account)
  ...
  const onDepositClick = async () => {
    try {
      if (Number(inputValue) > 0) {
        if (poolInfo?.token) {
          let response = await lpContract.allowance(account, bathFinance.GENESISFARM.address)
          let allowedAmount = Number(getFullDisplayBalance(response, lpContract.decimal));
          if (allowedAmount < Number(inputValue)) {
            let response = await lpContract.approve(bathFinance.GENESISFARM.address, ethers.constants.MaxUint256);
          ...
          let genesisFarm = bathFinance.GENESISFARM.connect(signer);
