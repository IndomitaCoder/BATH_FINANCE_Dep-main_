...
const SaunaAction = ({ poolInfo, pId }) => {
  ...
  const [pendingWETH, setpendingWETH] = useState(0);
  ...
  const fetchData = async () => {
    try {
      ...
      let res_p = await bathFinance.SAUNA.pendingwETHs(pId, account)
      let res_d = await bathFinance.SAUNA.userInfo(pId, account);
      setlpBalance(Number(getFullDisplayBalance(res_b, lpContract.decimal)));
      setlpSymbol(res_s);
      setpendingWETH(Number(getFullDisplayBalance(res_p, lpContract.decimal)))
  ...
  const onDepositClick = async () => {
    try {
      if (Number(inputValue) > 0) {
        if (poolInfo?.token) {
          let response = await lpContract.allowance(account, bathFinance.SAUNA.address)
          let allowedAmount = Number(getFullDisplayBalance(response, lpContract.decimal));
          if (allowedAmount < Number(inputValue)) {
            let response = await lpContract.approve(bathFinance.SAUNA.address, ethers.constants.MaxUint256);
          ...
          let saunaFarm = bathFinance.SAUNA.connect(signer);
          response = await saunaFarm.deposit(pId, amountBn)
          await addTransaction(response, {
  ...
  const onWithdrawClick = async () => {
    try {
      if (Number(inputValue) > 0) {
        const amountBn = parseUnits(Number(inputValue).toString(), poolInfo?.token.decimal)
        let saunaFarm = bathFinance.SAUNA.connect(signer);
        let response = await saunaFarm.withdraw(pId, amountBn);
        await addTransaction(response, {
        ...
        <Container maxWidth="lg">
          <Box mt={5} width={`100%`} style={{ display: "flex", flexDirection: 'column', backgroundColor: 'rgba(1,1,1,0.2)', padding: '50px 20px', borderRadius: '20px', backdropFilter: 'blur(15px)', margin: 0 }}>
            <div style={{ color: 'white', display: 'flex', flexFlow: 'wrap', flexDirection: 'column-reverse' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                <p style={{ margin: 0 }}>Pending wETH: &nbsp;</p>
                <p style={{ fontSize: 'larger', margin: 0 }}>{pendingWETH + ' wETH'}</p>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', position: 'relative' }}>
              ...
              {!isDeposit ? <p style={{ color: 'white', textAlign: 'center' }}>There is a {Number(poolInfo?.withdrawFee) / 100}% withdraw fee for this staking pool.</p> : null}
            </div>
          </Box>
        </Container>
      </Route>
