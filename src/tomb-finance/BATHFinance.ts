...
import { Fetcher, Route, Token, ChainId } from '@spookyswap/sdk';
import { Configuration } from './config';
import { ContractName, TokenStat, AllocationTime, LPStat, Bank, PoolStats, TShareSwapperStat, PoolInfo, PoolUserInfo } from './types';
...
import GenesisFarmABI from './GenesisFarm.abi.json'
import SteamRoomABI from './SteamRoom.abi.json'
import JacuzziABI from './Jacuzzi.abi.json'
import SaunaABI from './Sauna.abi.json'
import config, { bankDefinitions } from '../config';
import moment from 'moment';
import { parseUnits } from 'ethers/lib/utils';
import { FTM_TICKER, SPOOKY_ROUTER_ADDR, TOMB_TICKER } from '../utils/constants';
/**
 * An API module of BATH Finance contracts.
 * All contract-interacting domain logic should be defined in here.
 */
export class BATHFinance {
  myAccount: string;
  ...
  constructor(cfg: Configuration) {
    ...
    //////////////////////////////////
    this.GENESISFARM = new Contract(externalTokens['GENESIS'][0], GenesisFarmABI, provider);
    this.STEAMROOM = new Contract(externalTokens['STEAMROOM'][0], SteamRoomABI, provider)
    this.JACUZZI = new Contract(externalTokens['JACUZZI'][0], JacuzziABI, provider)
    this.SAUNA = new Contract(externalTokens['SAUNA'][0], SaunaABI, provider)
    /////////////////////////////////
    this.externalTokens = {};
  ...
  async getPoolLength(): Promise<Number> {
    this.GENESISFARM = this.GENESISFARM.connect(this.signer);
    const length: BigNumber = await this.GENESISFARM.poolLength();
    return Number(length);
  }

  async getTotalAllocPoint(): Promise<Number> {
    this.GENESISFARM = this.GENESISFARM.connect(this.signer);
    const length = await this.GENESISFARM.totalAllocPoint();
    return Number(length);
  }

  async getPoolInfoByPid(pId: Number): Promise<PoolInfo> {
    this.GENESISFARM = this.GENESISFARM.connect(this.signer);
    const info: PoolInfo = await this.GENESISFARM.poolInfo(pId);
    return info;
  }

  async getPoolUserInfoByPidAndAddress(pId: Number, account: string): Promise<PoolUserInfo> {
    this.GENESISFARM = this.GENESISFARM.connect(this.signer);
    const info: PoolUserInfo = await this.GENESISFARM.userInfo(pId, account);
    return info;
  }

  async depositInGenesis(pId: Number, amount: BigNumber): Promise<any> {
    this.GENESISFARM = this.GENESISFARM.connect(this.signer);
    return await this.GENESISFARM.deposit(pId, amount);
  }
  ...
  async getPoolLength1(): Promise<Number> {
    this.STEAMROOM = this.STEAMROOM.connect(this.signer);
    const length: BigNumber = await this.STEAMROOM.poolLength();
    return Number(length);
  }

  async getTotalAllocPoint1(): Promise<Number> {
    this.STEAMROOM = this.STEAMROOM.connect(this.signer);
    const length = await this.STEAMROOM.totalAllocPoint();
    return Number(length);
  }

  async getPoolInfoByPid1(pId: Number): Promise<PoolInfo> {
    this.STEAMROOM = this.STEAMROOM.connect(this.signer);
    const info: PoolInfo = await this.STEAMROOM.poolInfo(pId);
    return info;
  }

  async getPoolUserInfoByPidAndAddress1(pId: Number, account: string): Promise<PoolUserInfo> {
    this.STEAMROOM = this.STEAMROOM.connect(this.signer);
    const info: PoolUserInfo = await this.STEAMROOM.userInfo(pId, account);
    return info;
  }

  async depositInGenesis1(pId: Number, amount: BigNumber): Promise<any> {
    this.STEAMROOM = this.STEAMROOM.connect(this.signer);
    return await this.STEAMROOM.deposit(pId, amount);
  }
  ...
  async getPoolLength2(): Promise<Number> {
    this.JACUZZI = this.JACUZZI.connect(this.signer);
    const length: BigNumber = await this.JACUZZI.poolLength();
    return Number(length);
  }

  async getTotalAllocPoint2(): Promise<Number> {
    this.JACUZZI = this.JACUZZI.connect(this.signer);
    const length = await this.JACUZZI.totalAllocPoint();
    return Number(length);
  }

  async getPoolInfoByPid2(pId: Number): Promise<PoolInfo> {
    this.JACUZZI = this.JACUZZI.connect(this.signer);
    const info: PoolInfo = await this.JACUZZI.poolInfo(pId);
    return info;
  }

  async getPoolUserInfoByPidAndAddress2(pId: Number, account: string): Promise<PoolUserInfo> {
    this.JACUZZI = this.JACUZZI.connect(this.signer);
    const info: PoolUserInfo = await this.JACUZZI.userInfo(pId, account);
    return info;
  }

  async depositInGenesis2(pId: Number, amount: BigNumber): Promise<any> {
    this.JACUZZI = this.JACUZZI.connect(this.signer);
    return await this.JACUZZI.deposit(pId, amount);
  }
  ...
  async getPoolLength3(): Promise<Number> {
    this.SAUNA = this.SAUNA.connect(this.signer);
    const length: BigNumber = await this.SAUNA.poolLength();
    return Number(length);
  }

  async getTotalAllocPoint3(): Promise<Number> {
    this.SAUNA = this.SAUNA.connect(this.signer);
    const length = await this.SAUNA.totalAllocPoint();
    return Number(length);
  }

  async getPoolInfoByPid3(pId: Number): Promise<PoolInfo> {
    this.SAUNA = this.SAUNA.connect(this.signer);
    const info: PoolInfo = await this.SAUNA.poolInfo(pId);
    return info;
  }

  async getPoolUserInfoByPidAndAddress3(pId: Number, account: string): Promise<PoolUserInfo> {
    this.SAUNA = this.SAUNA.connect(this.signer);
    const info: PoolUserInfo = await this.SAUNA.userInfo(pId, account);
    return info;
  }

  async depositInGenesis3(pId: Number, amount: BigNumber): Promise<any> {
    this.SAUNA = this.SAUNA.connect(this.signer);
    return await this.SAUNA.deposit(pId, amount);
  }
